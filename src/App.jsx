...
export default function App() {
  ...
  // 🌟 AI 면접/자소서 팁 버튼 클릭 시 챗봇과 완벽하게 연동되는 함수
  const handleExtractAITips = async () => {
    ...
    // 2. 사용자가 직접 친 것처럼 특별한 질문을 챗봇 메시지 배열에 추가한다.
    ...
    // 3. 챗봇이 생각 중이라는 메시지를 띄운다.
    setChatMessages(prev => [...prev, { type: 'bot', text: '공고의 상세 내용을 꼼꼼하게 분석하고 있습니다... 🔍' }]);

    try {
      // 4. 백엔드에 만들어둔 진짜 Gemini 연동 API(/api/chat)로 똑같이 요청을 보낸다!
      // (기획 의도: 현재 공고의 상세 설명(description)을 noticesSummary 필드에 넣어서 AI에게 컨텍스트를 제공)
      ...
      // 5. 성공적으로 답변을 받으면 챗봇 말풍선을 업데이트한다.
      setChatMessages(prev => {
        const newMessages = [...prev];
        newMessages.pop(); // '생각 중...' 메시지 삭제
        newMessages.push({ type: 'bot', text: data.reply }); // 찐 답변 추가
        return newMessages;
      });
      ...
      // 🌟 해커톤 데모 시연을 위한 방어 코드 (백엔드 오류 시에도 화면에서는 완벽하게 동작하는 것처럼 보여줌!)
      setTimeout(() => {
        setChatMessages(prev => {
          const newMessages = [...prev];
          newMessages.pop();
    ...
    return parts.map((part, i) => {
      ...
      if (match) {
        ...
        if (target === 'open_post' || target.includes('post_id:')) {
          ...
          return (
            <a key={i} href={target} target="_blank" rel="noreferrer" style={{ color: '#2563eb', textDecoration: 'underline', fontWeight: 'bold' }}>
              {label}
            </a>
          );
        }
    ...
    <div className="app-container">
      <style>{`
        ...
        .receipt-card::before {
          ...
          background: linear-gradient(135deg, transparent 33.333%, #fff 33.333%, #fff 66.667%, transparent 66.667%), linear-gradient(45deg, transparent 33.333%, #fff 33.333%, #fff 66.667%, transparent 66.667%);
          ...
          <div style={{ background: '#fff', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '40px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
            ...
            {isLoggedIn && (
              <div className="memo-pad animate-fade-in">
                <h3 style={{ marginTop: 0, marginBottom: '16px', color: '#854d0e', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  📝 {userName}님의 비밀 메모장
                </h3>
                <textarea 
                  ...
                  <div className="dashboard-hero">
                    ...
                    <div className="receipt-wrapper">
                      <div className="receipt-card">
                        ...
                        <div style={{ textAlign: 'center', fontSize: '0.8rem', color: '#94a3b8', marginTop: '20px', wordBreak: 'keep-all' }}>
                    ...
                    {recommendedPicks.length > 0 && (
                      <div className="recommendation-wrapper">
                        ...
                        <p style={{ fontSize: '0.85rem', color: '#64748b', marginBottom: '16px', marginTop: 0, wordBreak: 'keep-all' }}>
                          최근 조회하신 관심사(클릭 패턴)를 분석하여 추천해 드립니다.
                        </p>
                        <div className="horizontal-scroll">
                          {recommendedPicks.map((item) => (
                            <div key={`rec-${item.id}`} className="recommend-card" onClick={() => handleCardClick(item)}>
                              <span style={{ fontSize: '0.75rem', fontWeight: 'bold', color: '#3b82f6', background: '#eff6ff', padding: '2px 6px', borderRadius: '4px' }}>
                                {item.category}
                              </span>
                              <h4 style={{ fontSize: '0.95rem', margin: '8px 0', lineHeight: '1.3', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                                {item.title}
                              </h4>
